...
the storage modules linked to the x-center via ``via_device``.
"""
...
def _build_device_infos(entry: KermiModbusConfigEntry) -> dict[str, DeviceInfo]:
    """One DeviceInfo per configured unit (project plan §10.1).
    ...
    if entry.data.get(CONF_HEATING_CIRCUIT_UNIT) is not None:
        device_infos[DEVICE_KEY_HEATING_CIRCUIT] = DeviceInfo(
            ...
            via_device=(DOMAIN, xcenter_identifier),
        )
...
async def async_setup_entry(hass: HomeAssistant, entry: KermiModbusConfigEntry) -> bool:
    ...
    entry.runtime_data = KermiModbusData(
        ...
        device_infos=_build_device_infos(entry),
    )
